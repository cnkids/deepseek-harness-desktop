...
`DSH_DESKTOP_NODE_MIRROR` 指向 Node.js 发行目录（不含文件名），`DSH_DESKTOP_NPM_REGISTRY` 指向 npm registry 根地址。软件源会决定装的是哪个包、哪个运行时，因此覆盖值只接受 `https://`（本机 `http://127.0.0.1`、`http://localhost` 私有源例外），其他值会被忽略并回退到官方源；日志与启动页只显示源的主机名，不会带出 URL 里的账号密码。

## Harness 更新提示

`@deepseek-ai/dsh` 的版本同步不再"悄无声息地装完就走"：

- **启动时**：发现新版本会弹窗询问一次，可选「立即更新」（启动页显示更新阶段与已用时）或「稍后」（先用当前版本启动，不耽误你干活）。同一个版本只会询问一次，选择稍后之后下次启动不会再弹。
- **应用常驻不重启时**：每 6 小时复查一次。发现新版本会提示一次，也可以在系统托盘菜单里随时手动更新——托盘常驻一行「检查 Harness 更新（当前 vX）」，有新版本时变成「更新 Harness 到 vY」。
- **更新过程**：先停止当前 Harness 再执行 npm 安装（避免 Windows 下覆盖正在运行的文件失败），完成后自动重启 Harness，当前页面会重新加载。更新失败会保留当前版本继续运行，并在 5 分钟内不再自动重试，可稍后从托盘重试。
- 首次安装（本机还没有 `dsh`）仍然是静默完成，因为此时没有可用的旧版本。
...
| 供应链校验 | 软件源只接受 `https://`（本机 http 私有源例外）；版本号必须通过 semver 校验后才会进入 npm 安装参数；日志与界面展示源地址时脱敏账号密码 |
| 自动同步 | 启动时查询 npm registry（官方源优先，不可达时自动改用国内镜像），发现新版本时询问一次；应用常驻期间每 6 小时复查，托盘随时可以手动更新，详见[Harness 更新提示](#harness-更新提示) |
...
- 同一个版本只主动提示一次（记录在 `userData/cache/dsh-update-notice.json`），所以"选了稍后"不会再每次启动都被弹窗打扰；托盘入口始终可以手动更新。
- 更新过程先停止 Harness 再执行 npm 安装（避免 Windows 下覆盖正在运行的 dsh 文件失败），装完自动重启 Harness；npm 拿不到精确百分比，因此启动页显示更新阶段与已用时，避免看起来像卡死。失败时保留当前版本继续运行，5 分钟内不自动重试，可稍后从托盘重试。
